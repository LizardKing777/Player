...
constexpr Drawable::Z_t default_priority = Priority_Player; // Default to character layer.
...
void Stream::draw_block(Bitmap& dst, int ref, uint8_t n, uint8_t z, uint8_t c0, int16_t cam_x, int16_t cam_y) {
	bool is_mode7 = Game_Map::GetIsMode7() && !isScreenRelative && renderType != RenderType::Screen;

	if (is_mode7 && renderType == RenderType::Sprite) {
		// --- SPRITE PLANE LOGIC FOR STREAM ---
		// Transform the stream's origin ONCE, then draw all particles relative to it.
		int origin_ref_idx = ref / (amount * fade);

		// Calculate screen position of the stream's origin BEFORE transforming
		int origin_screen_x = str_x[origin_ref_idx] - cam_x;
		int origin_screen_y = str_y[origin_ref_idx] - cam_y;
		auto origin_transform = Game_Map::TransformToMode7(origin_screen_x, origin_screen_y);
		float y_adjust = 0.0f;

		// ** HORIZON RECOVERY **
		// If the origin is clipped (zoom <= 0), it might be right at the horizon.
		// Try a fallback point slightly closer to the camera (South) to get a valid zoom.
		if (origin_transform.zoom <= 0) {
			auto fallback_transform = Game_Map::TransformToMode7(origin_screen_x, origin_screen_y + 16); // Check 16 pixels down
			if (fallback_transform.zoom > 0) {
				// Use the fallback scale but assume original X/Y for billboarding anchor
				origin_transform.zoom = fallback_transform.zoom;
				origin_transform.screen_x = fallback_transform.screen_x;
				origin_transform.screen_y = fallback_transform.screen_y;
				// Adjust rendering upward to compensate for the fallback offset
				y_adjust = -16.0f * origin_transform.zoom;
			} else {
				return; // Still clipped, give up
			}
		}
		...
		for (uint8_t i = 0; i < n; i++) {
			...
			for (int j = 0; j < amount; j++) {
				...
				// Calculate particle's offset from its origin in world space
				float particle_offset_x = x[p_idx] - str_x[origin_ref_idx];
				float particle_offset_y = y[p_idx] - str_y[origin_ref_idx];

				// Apply the origin's zoom to the offset and add to the transformed origin's screen position
				...
				float size = s[p_idx] * origin_transform.zoom;

				Rect dst_rect(final_x - size / 2.0f, final_y - size / 2.0f, size, size);

				if (hasTexture) {
					// Use StretchBlit for textured particles
	...
	} else if (is_mode7 && renderType == RenderType::Map) {
		// --- MAP PLANE LOGIC FOR STREAM (Transform each particle) ---
		for (uint8_t i = 0; i < n; i++) {
			int age = i + c0;
			if (age >= fade) continue;
			...
			for (int j = 0; j < amount; j++) {
				...
				int screen_x = x[p_idx] - cam_x;
				int screen_y = y[p_idx] - cam_y;
				auto transform = Game_Map::TransformToMode7(screen_x, screen_y);

				if (transform.zoom > 0) {
					float size = s[p_idx] * transform.zoom;
	...
	if (Game_Map::GetIsMode7() && !isScreenRelative && renderType != RenderType::Screen) {
		for (int i = 0; i < simulCnt; i++) {
			...
			if (renderType == RenderType::Sprite) {
				// --- SPRITE PLANE RENDERING ---
				int input_x = origins[i].x - cam_x;
				int input_y = origins[i].y - cam_y;
				auto origin_transform = Game_Map::TransformToMode7(input_x, input_y);
				float y_adjust = 0.0f;

				Drawable::Z_t z = base_z + z_offset;
				if (origin_transform.zoom > 0) {
					z += (static_cast<Drawable::Z_t>(origin_transform.screen_y) + 0x8000) << 32;
					z += (static_cast<Drawable::Z_t>(origin_transform.screen_x) + 0x8000) << 16;
				}
				SetZ(z);
